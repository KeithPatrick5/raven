...
function materialityFor(term: RegulatoryWatchTerm, doc: FederalRegisterDoc): RegulatorySignal["materiality"] {
  const type = compact(doc.type).toLowerCase();
  ...
  if (text.includes("proposed rule") || text.includes("final rule") || text.includes("sanction") || text.includes("tariff")) return "possibly_material";
  return "routine";
}

function actionFor(materiality: RegulatorySignal["materiality"], confidence: number): Pick<RegulatorySignal, "action" | "status"> {
...
function buildSignals(term: RegulatoryWatchTerm, docs: FederalRegisterDoc[]) {
  const signals: RegulatorySignal[] = [];

  for (const doc of docs) {
    if (!doc.document_number || !doc.title) continue;
    const materiality = materialityFor(term, doc);
    const confidence = Math.max(0, Math.min(100, term.confidence + (materiality === "possibly_material" ? 6 : 0)));
    const { action, status } = actionFor(materiality, confidence);
    ...
    for (const ticker of term.tickers) {
      signals.push({
        ...
        action,
        status,
        rawPayload: { ...doc, matchedTerm: term.term, matchedCategory: term.category, ticker }
      });
    }
  }
    ...
    try {
      await upsertSignalEvent({
        ...
        metadata: {
          ...
          note: "Federal Register matches are sector or policy context. Raven treats them as watch signals unless price and stronger catalysts confirm."
        }
      });
...
export async function scanFederalRegisterSignals() {
  ...
  const signalsByKey = new Map<string, RegulatorySignal>();

  for (const term of REGULATORY_WATCH_TERMS) {
    try {
      const fetched = await fetchFederalRegisterDocs(term);
      for (const signal of buildSignals(term, fetched.docs)) {
        const key = `${signal.documentNumber}:${signal.ticker}:${signal.term}`;
        if (!signalsByKey.has(key)) signalsByKey.set(key, signal);
      }
    } catch (error) {
  ...
  const signals = [...signalsByKey.values()]
  ...
  return {
    ok: errors.length === 0 || signals.length > 0,
    ...
    errors: [...errors, ...storage.errors, ...eventResult.errors]
  };
}
