...
export async function ensureRavenTables() {
  ...
  await sql`alter table paper_trades add column if not exists client_order_id text`;
  ...
  await sql`alter table paper_trades add column if not exists max_hold_days integer not null default 5`;
